...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    float height = ImGui::GetFrameHeight(); // Checkbox size based on frame height
    float side_length = height;  // Side length for the square checkbox
    ...
    // Create an invisible button for the checkbox area
    ImGui::InvisibleButton(label, ImVec2(side_length, side_length));
    if (ImGui::IsItemClicked())
        *v = !*v; // Toggle the checkbox state

    // Animation logic for smooth transition
    float t = *v ? 1.0f : 0.0f;
    ...
    if (ImGui::IsItemHovered())
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.12f, 0.20f, 0.32f, 1.f), ImVec4(0.12f, 0.60f, 1.f, 1.f), t));
    else
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(0.12f, 0.20f, 0.32f, 1.f), ImVec4(0.12f, 0.60f, 1.f, 1.f), t));

    // Draw the outer rectangle
    draw_list->AddRect(p, p + ImVec2(side_length, side_length), col_bg, 0.0f, ImDrawFlags_None, 2.0f); // Outline thickness set to 2.0f

    // Draw the inner filled rectangle only if checked
    if (*v)
    {
        ImU32 col_fill = ImColor(236, 174, 255); // Fill color (white)
        draw_list->AddRectFilled(p + ImVec2(2.0f, 2.0f), p + ImVec2(side_length - 2.0f, side_length - 2.0f), col_fill); // Slight padding for filled rectangle
    }

    // Draw the label if present
    if (label_size.x > 0.0f)
    {
        RenderText(ImVec2(pos.x + height + style.ItemInnerSpacing.x, pos.y + style.FramePadding.y), label);
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	if (label_size.x > 0)
        // modified by asphyxia
		RenderText(ImVec2(bb.Min.x, total_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_FrameBg), true, g.Style.FrameRounding);
