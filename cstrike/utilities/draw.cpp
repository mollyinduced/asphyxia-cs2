...
bool ImGui::ColorEdit4(const char* szLabel, ColorPickerVar_t* pColorVar, ImGuiColorEditFlags flags)
{
	...
	// @todo: cleaner code
	SetNextWindowSize(ImVec2((pColorVar->bRainbow ? 120.f : 60.f) * D::CalculateDPI(g_Config::nDpiScale), 0.f));
	if (BeginPopup(CS_XOR("context##color.picker")))
	{
		if (Button(CS_XOR("copy##color.picker"), ImVec2(-1, 15 * D::CalculateDPI(g_Config::nDpiScale))))
		{
		...
		if (Button(CS_XOR("paste##color.picker"), ImVec2(-1, 15 * D::CalculateDPI(g_Config::nDpiScale))))
		{
...
bool D::Setup(HWND hWnd, ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	for (int i = 0; i < CS_ARRAYSIZE(FONT::pMenu); i++)
	{
		...
		FONT::pMenu[i] = io.Fonts->AddFontFromMemoryTTF(fataFont, sizeof(fataFont), 12); //io.Fonts->AddFontFromFileTTF(CS_XOR("C:\\Windows\\Fonts\\Verdana.ttf"), flFontSize, &imVerdanaConfig, io.Fonts->GetGlyphRangesCyrillic());
	...
	espFontConfig.FontBuilderFlags = ImGuiFreeTypeBuilderFlags_LightHinting;
	FONT::pEsp = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\tahomabd.ttf", 12, &espFontConfig);

	imVerdanaConfig.FontBuilderFlags = ImGuiFreeTypeBuilderFlags_Bold;
	FONT::pExtra = io.Fonts->AddFontFromMemoryTTF(fataFont, sizeof(fataFont), 12); //io.Fonts->AddFontFromFileTTF(CS_XOR("C:\\Windows\\Fonts\\Verdana.ttf"), 12.f, &imVerdanaConfig, io.Fonts->GetGlyphRangesCyrillic());

	ImFontConfig imTahomaConfig;
	imTahomaConfig.FontBuilderFlags = ImGuiFreeTypeBuilderFlags_LightHinting;
	FONT::pVisual = io.Fonts->AddFontFromMemoryTTF(fataFont, sizeof(fataFont), 12); //io.Fonts->AddFontFromFileTTF(CS_XOR("C:\\Windows\\Fonts\\Tahoma.ttf"), 12.f, &imTahomaConfig, io.Fonts->GetGlyphRangesCyrillic());
...
bool D::OnWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	// switch menu state
	if (IPT::IsKeyReleased(g_Config::nMenuKey))
	{
