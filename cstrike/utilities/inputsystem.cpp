...
bool IPT::Setup()
{
	...
	pOldWndProc = reinterpret_cast<WNDPROC>(SetWindowLongPtrW(hWindow, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(hooks::WndProc)));
...
bool IPT::OnWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam != g_Config::nMenuKey && wParam != g_Config::nPanicKey && MENU::bMainWindowOpened)
		return false;
