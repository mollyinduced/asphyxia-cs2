...
bool isCS2Ready() {
	HWND cs2Win = FindWindow(L"SDL_app", L"Counter-Strike 2");
	if (!cs2Win)
		return false;
	if (!GetModuleHandle(NAVSYSTEM_DLL))
		return false;
	if (!GetModuleHandle(MATERIAL_SYSTEM2_DLL))
		return false;
	return true;
}
...
static bool Setup(HMODULE hModule)
{
	...
	if (!L::AttachConsole(CS_XOR(L"gamerware developer console")))
	{
...
#endif
#ifdef CS_LOG_FILE
	if (!L::OpenFile(CS_XOR(L"gamerware.log")))
	{
...
DWORD WINAPI PanicThread(LPVOID lpParameter)
{
	...
	while (!IPT::IsKeyReleased(g_Config::nPanicKey))
		::Sleep(500UL);
