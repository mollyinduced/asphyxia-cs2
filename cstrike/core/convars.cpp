...
bool CONVAR::Dump(const wchar_t* wszFileName)
{
	...
	CRT::String_t<64> szTimeBuffer(CS_XOR("[%d-%m-%Y %T] gamerware | convars dump\n\n"), &timePoint);
...
bool CONVAR::Setup()
{
	...
	sv_friction = I::Cvar->Find(FNV1A::HashConst("sv_friction")); // flaot
	bSuccess &= sv_friction != nullptr;

	sv_accelerate = I::Cvar->Find(FNV1A::HashConst("sv_accelerate")); // flaot
	bSuccess &= sv_accelerate != nullptr;

	weapon_accuracy_shotgun_spread_patterns = I::Cvar->Find(FNV1A::HashConst("weapon_accuracy_shotgun_spread_patterns")); // flaot
	bSuccess &= weapon_accuracy_shotgun_spread_patterns != nullptr;	

	weapon_accuracy_nospread = I::Cvar->Find(FNV1A::HashConst("weapon_accuracy_nospread")); // flaot
	bSuccess &= weapon_accuracy_nospread != nullptr;
	
	sv_maxunlag = I::Cvar->Find(FNV1A::HashConst("sv_maxunlag")); // flaot
