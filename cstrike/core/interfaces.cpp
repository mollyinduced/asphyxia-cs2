...
#pragma region interface_game_exported
	...
	Prediction = Capture<CPrediction>(pClientRegister, C_PREDICTION);
	if (Prediction)
		L_PRINT(LOG_WARNING) << "CLIENT@Source2ClientPrediction001: " << Prediction;
...
#pragma endregion
	...
	if (!EngineTrace)
		EngineTrace = *reinterpret_cast<C_EngineTrace**>(MEM::GetAbsoluteAddress(MEM::FindPattern(CLIENT_DLL, CS_XOR("4C 8B 3D ? ? ? ? 24 ? 0C ? 89 5D 64")), 0x3));
	bSuccess &= (EngineTrace != nullptr);
