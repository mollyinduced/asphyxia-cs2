...
inline int HitboxToBone(uint16_t hitboxIndex)
{
	if (hitboxIndex == HEAD)
		return head;

	else if (hitboxIndex == NECK)
		return neck_0;

	else if (hitboxIndex == PELVIS)
		return pelvis;

	else if (hitboxIndex == SPINE_0)
		return spine_1;

	else if (hitboxIndex == SPINE_1)
		return spine_1;

	else if (hitboxIndex == SPINE_2)
		return spine_2;

	else if (hitboxIndex == SPINE_3)
		return spine_2;

	else if (hitboxIndex == LEG_UPPER_L)
		return leg_upper_L;

	else if (hitboxIndex == LEG_UPPER_R)
		return leg_upper_R;

	else if (hitboxIndex == LEG_LOWER_L)
		return leg_lower_L;

	else if (hitboxIndex == LEG_LOWER_R)
		return leg_lower_R;

	else if (hitboxIndex == ANKLE_L)
		return ankle_L;

	else if (hitboxIndex == ANKLE_R)
		return ankle_R;

	else if (hitboxIndex == HAND_L)
		return hand_L;

	else if (hitboxIndex == HAND_R)
		return hand_R;

	else if (hitboxIndex == ARM_UPPER_L)
		return arm_upper_L;

	else if (hitboxIndex == ARM_LOWER_L)
		return arm_lower_L;

	else if (hitboxIndex == ARM_UPPER_R)
		return arm_upper_R;

	else if (hitboxIndex == ARM_LOWER_R)
		return arm_lower_R;
}

inline QAngle_t CalcAngle(Vector_t viewPos, Vector_t aimPos)
{
	QAngle_t angle = { 0, 0, 0 };

	Vector_t delta = aimPos - viewPos;

	angle.x = -asin(delta.z / delta.Length()) * (180.0f / M_PI);
...
struct RageEntity
{
	...
	QAngle_t vecCalcAimPos;
	Vector_t vecAimPosNonCalc;
	LagRecord_t* pRecord;
	float flHealth;
	float flDamage;
	std::vector<Vector_t> vecMultiPoints;
};
...
namespace RAGEBOT
{
	inline std::vector<RageEntity> RageEntities;
	inline Ragebot cRagebot;
	inline RageEntity BestEntity;
	...
	ShootPoint GetShootPoint(LagRecord_t* pRecord, CModelState pModel);
